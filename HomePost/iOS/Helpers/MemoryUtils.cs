...
namespace HomePost.iOS.Helpers
{
	...
	public static class MemoryUtils
	{
		...
		public static async void DelayReleaseObject(int milisecond, IDisposable disposableObject, bool enableSelfCleaning = true)
		{
			await Task.Delay(milisecond);
			ReleaseObject(disposableObject, enableSelfCleaning);
		}

		public static async void DelayReleaseObject<T>(int milisecond, WeakReference<T> weakReference, bool enableSelfCleaning = true) where T : class, IDisposable
		{
			await Task.Delay(milisecond);
			ReleaseObject(weakReference, enableSelfCleaning);
		}
		...
		public static void ReleaseUICollectionView(IDisposable disposableObject)
		{
			var collection = disposableObject as UICollectionView;

			if (collection == null || collection.NumberOfSections() == 0)
				return;

			nint rows = collection.NumberOfItemsInSection(0);

			for (nint i = 0; i < collection.NumberOfSections(); i++)
			{
				for (nint ir = 0; ir < collection.NumberOfItemsInSection(i); ir++)
				{
					var cell = collection.CellForItem(Foundation.NSIndexPath.FromRowSection(ir, i));
					if (cell == null)
						return;

					ReleaseUIViewWithChildren(cell.BackgroundView, true);
					ReleaseUIViewWithChildren(cell.SelectedBackgroundView, true);
					ReleaseUIViewWithChildren(cell.ContentView, true);
				}
			}
		}
